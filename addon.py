...
# Default Hyper3D free trial key — migrate to config_new.py APIKeys.hyper3d_free_trial_key
...
def get_action_fcurves(action):
    ...
    list
        ...
        Never raises — returns [] on any access error.
    """
    if action is None:
        return []
    ...
    try:
        ...
        pass  # attribute does not exist — falls through to 5.x path
    ...
    try:
        ...
        pass  # any structural mismatch → empty
...
def register():
    ...
    # Auto-start the server when addon is registered
    import threading
    import time
    
    def auto_start_server():
        time.sleep(1.0)  # Wait for Blender to fully initialize
        try:
            if not hasattr(bpy.types, "blendermcp_server") or bpy.types.blendermcp_server is None:
                port_val = bpy.context.scene.blendermcp_port
                bpy.types.blendermcp_server = BlenderMCPServer(port=port_val)
                print(f"BlenderMCP server instance created on port {port_val}")
            
            if not bpy.types.blendermcp_server.running:
                bpy.types.blendermcp_server.start()
                bpy.context.scene.blendermcp_server_running = True
                print(f"BlenderMCP auto-started on port {bpy.types.blendermcp_server.port}")
            else:
                print("BlenderMCP server already running")
        except Exception as e:
            print(f"BlenderMCP auto-start failed: {str(e)}")
            import traceback
            traceback.print_exc()
    
    # Start auto-start in a separate thread
    auto_thread = threading.Thread(target=auto_start_server, daemon=True)
    auto_thread.start()
